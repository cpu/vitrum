...
import (
	...
	"github.com/usbarmory/tamago/soc/nxp/usdhc/internal/blockcount"
)
...
func (hw *USDHC) transfer(index uint32, dtd uint32, arg uint64, blocks uint32, blockSize uint32, buf []byte) (err error) {
	...
	blockCount, err := blockcount.Decode(blocks, hw.rpmb)
	if err != nil {
		return err
	}
	...
	// set block count
	reg.SetN(hw.blk_att, BLK_ATT_BLKCNT, 0xffff, blockCount)
